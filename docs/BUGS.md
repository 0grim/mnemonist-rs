...
This document is the Bug Catcher submission: defects in the JavaScript library this project ports,
identified by reading its source line by line and by differentially fuzzing it against the Rust
...
**How the port handles each one.** With two exceptions noted inline, the port reproduces every bug
below bug-for-bug rather than fixing it. `mnemonist-core`'s contract is to behave like `mnemonist`,
bugs included: the JavaScript library's own test suite is the evidence of equivalence, and a port
that silently repairs a bug the suite doesn't test for is no longer verifiably equivalent to it. In
a few places fidelity cost idiomatic Rust: `MultiSet`'s size counter is tracked rather than derived
so that a delete-on-absent-key bug (below) reproduces instead of self-healing, and one bridge
reports `null` for a falsy evicted key because upstream's own truthiness check does the same.

## How these are ranked

A defect whose upstream test asserts the buggy output as correct would show that the bug survived
review; a defect the tests never reach shows only that nobody exercised that state. Across every bug
below, the existing test suite never exercises the state where the bug lives — no case of the former
kind was found. The ranking here is by severity first (silent data corruption above a crash, a crash
above a wrong error message), then by how narrowly the existing suite missed it, then by how
...
Member 5 now reports member 3's value, on the very first `delete` from a map holding more than one
entry.
...
map holds exactly one entry, where the swap-with-last is a self-assignment indistinguishable from
correct behavior. Confirmed by sabotaging the port to fix this bug: `test/sparse-map.js` stays at 9
passing, 0 failing, four of the port's native tests turn red, and the differential fuzzer (comparing
the Rust port against real upstream JS) catches the divergence in 3.0 seconds, minimized to a
three-operation repro.

**How the port handles it.** Reproduced verbatim: `SparseMap::delete` performs the identical
swap-with-last on the key/index arrays, leaving the value array's stale slot in place.
...
```
The correct suffix array for that string is `[0,1,2,5,3,12,6,14,4,11,13,10,9,8,7]`; two positions
(`6,4,14` vs `6,14,4`) are transposed.
...
falls through to 8 bits, so the sort ends up comparing only the low byte of every 16-bit character
code, for every symbol above U+00FF whose low byte collides with another symbol's. Measured over
10,000 random inputs (length 1–30, alphabet `{'A','Ł'}`, length divisible by 3): 81% produce a wrong
suffix array; pure-ASCII input is unaffected, since no low-byte collision exists in that range.

**Why the suite misses it.** Every string in `test/suffix-array.js` is ASCII. The bug requires a
non-Latin1 character whose low byte collides with another symbol in the same string, a condition
nothing in the existing suite reaches.

**How the port handles it.** Reproduced: the Rust port performs the identical radix-width
...
contribution and `b`'s high half for the high contribution. `sum32` has both halves backwards: it
adds `b`'s high half into `a`'s low half and vice versa, broken as a general 32-bit adder, as
confirmed above.

It is called exactly once in the file, with the constant `n = 0x6b64e654`. MurmurHash3's published
mixing constant is `0xe6546b64`, the same eight hex digits with the two 16-bit halves swapped. The
...
from is not. Reusing `sum32` elsewhere, or correcting the constant to the textbook value, breaks
every hash the library has ever produced.
...
observable symptom for any test to catch, deterministically, at this call site. This is not a case
of the suite failing to test the bug, but of there being nothing observable to test in the first
place, confirmed by swapping the constant to the textbook value: `test/bloom-filter.js` (the only
consumer) goes from 6/6 to 4/6 passing, while replacing `sum32(hash, N)` with plain
`hash + 0xe6546b64` leaves it at 6/6. The bug is required for the shipped constant to keep working,
and the shipped constant is required for the bug to be invisible. Neither survives being changed
alone.

**How the port handles it.** Reproduced verbatim, both halves pinned by tests.
...
`size` can exceed `length`, and since `values()`/spread freeze `size` while `dense` is a
fixed-length typed array, iterating past `length` yields `undefined` for every excess slot.

**Why the suite misses it.** `test/sparse-set.js` never calls `add` with a member at or above the
set's configured length.

**How the port handles it.** Reproduced: every step here is a well-defined typed-array read, a
...
Any key whose stored value is `undefined` makes every subsequent `get` treat the key as missing:
the factory re-runs (so a stateful factory, e.g. an auto-incrementing one, advances on a read), and
`size` grows once per read rather than once per entry. The drift is silent and self-healing — any
later `set` or `delete` recomputes `size` from `items.size` and snaps it back to correct — so an
interleaved program shows a `size` that is sometimes right, with nothing to distinguish which.

**Why the suite misses it.** None of the seven assertions in `test/default-map.js` reads a value of
`undefined` back out and calls `get` on that key a second time. Confirmed with this project's own
gate-6 falsification: sabotaging the port to use `items.has` instead of the value check — the more
correct behaviour — leaves the original 7-assertion suite at 7 passing, while the differential
fuzzer (comparing the port against real upstream) catches the divergence in 136 cases (0.1s),
minimised to two operations.

**How the port handles it.** Reproduced: the correction is what a careful porter writes by
accident, since deriving `size` from the entry count is tidier and matches what the name suggests —
which is exactly why it had to be checked against the real behaviour rather than assumed.
...
unlinked objects, so nothing from that point on is stored anywhere reachable from `root`. The
bookkeeping only sees that the final orphan object has no `SENTINEL` property of its own, which is
true and says nothing about whether the value went anywhere.

**Why the suite misses it.** Neither `test/trie.js` nor `test/trie-map.js` embeds the sentinel
character inside a real token — every key in both suites is an ordinary word.

**How the port handles it.** This is the one bug in this document the port does not reproduce.
...
`if (oldKey)` is JavaScript truthiness, not a check for whether an eviction was reported. Any falsy
key that gets evicted is reported identically to no eviction at all.

**Why the suite misses it.** All three `setpop` blocks in `test/lru-cache.js` evict or overwrite a
plain non-empty string. Found instead by the differential fuzzer on the first generated case for
this grammar — the fuzz key pool deliberately includes falsy values.

**How the port handles it.** Reproduced deliberately, at a cost to idiom: the port's core
...
stale. The drift heals itself on the next `set`/`delete` on either side, since both recompute both
counters from the live maps — except a `delete` of an absent key, which returns `false` before
touching either counter, so `clear()` immediately followed by a no-op `delete()` leaves the stale
counter stale for one extra operation.

**Why the suite misses it.** `test/bi-map.js` never reads `.inverse.size` after a `clear()`. Found
by the differential fuzzer, and worth recording as a caution about this project's own process: a
first attempt at the port derived both sizes from the underlying map's length, which healed the
desync — more correct than upstream and therefore itself a bug — caught by the fuzzer in 18 cases
(0.3s). A second attempt resynced both counters unconditionally after every `set`/`delete`, which
re-healed the one-extra-operation case above; caught in 177 cases (0.3s) on the next campaign. The
eventual port only resyncs on an actual removal, matching upstream's own conditional exactly.

**How the port handles it.** Reproduced, on the third attempt.
...
**Why the suite misses it.** `test/suffix-array.js` has one length-22 input (≡1 mod 3, nominally in
the affected class), but it is `'This is a long string.'`, which has no repeated trigram, so the
recursive branch this bug lives in never runs. The suite is one repeated trigram away from having
caught this.

**How the port handles it.** Reproduced: the DC3 recursion boundary is ported with the identical
omission.
...
**What upstream does wrong.** The constructor's own comment reads `// TODO: yell if capacity is
...
**How the port handles it.** Reproduced as a typed error (`Error::Corrupted`, carrying upstream's own
...
`heap.clear()`, which sets `size = 0` mid-call; the pending `this.size--` then computes `0 - 1`, a
value JavaScript holds without complaint. The next `pop()` doesn't see an empty heap either — its
guard is `if (!this.size) return undefined;`, and `-1` is truthy — so it proceeds into a heap whose
fields `clear()` already nulled and crashes several lines later.
...
**Why the suite misses it.** `test/fibonacci-heap.js` never uses a mutating comparator. This
project's own 12 native tests and 6-block boundary suite for this module also could not have caught
the sibling bug produced by the gate-6 falsification for this unit (flipping `push`'s tie-break
comparison) — the values it reorders are equal, so no expected-value assertion can observe the
difference. The differential fuzzer caught that one in 425 generated cases; a tie-break reordering
of equal values is visible only to a reference-implementation comparison, evidence that a fuzzer
and a test suite are not redundant instruments.

**How the port handles it.** Reproduced: `size` is a signed `i64`, not an unsigned type, specifically
...
tie branch, `ranks[2]` is never incremented, even though the merged tree now has four elements. The
rank array no longer reflects true subtree height at all.
...
increments a rank) fires on nearly every merge of two non-singleton trees — union-by-rank is
effectively disabled.

**Why this is included as a performance bug, ranked last.** It is the one defect in this document no
execution-based instrument could have found: `find()` returns the correct root regardless of which
tree got attached to which, so no differential comparison against upstream will ever disagree,
however many operations it runs — a limit on what a clean fuzz campaign can prove. Only reading the
source catches this class of bug.

**How the port handles it.** Reproduced: `find(x)` returns a root, and which element becomes root is
...
intentional design rather than a defect; the submission's credibility does not rest on them.
...
```
This may be intentional: `iter` must return a genuine iterator, while `forEach` only needs to visit
values. Even so, two helpers in the same small library disagreeing about the same input is at
minimum a documentation gap.
...
```
An empty string is a legitimately iterable value that should yield zero times, not throw. This may
be an intentional "reject nothing" guard that was not considered for this case.
...
[]
```
This is reproducible, though it requires an adversarial `toString`, and the practical impact is low.

## Judged not to be bugs

Two candidates were considered and left out of the claims above, because on inspection they are
unreachable through any public sequence of calls:

`Stack.prototype.values()` reads `this.items.length` instead of `this.size`. Every other method in
`stack.js` is written against `this.size`, so this reads as an inconsistency between the method and
the rest of the file. But `Stack` backs its storage with a genuine, unbounded JS `Array`, and both
`push` and `pop` mutate `items` and `size` in lockstep on every call — there is no code path in the
file that can make `items.length !== this.size`. The inconsistency is real in the source; it has no
observable consequence.

`obliterator/take`'s `n`-tracking guard (`if (i !== n) array.length = i`) is always true when `n` is
omitted, because `n` is then `undefined` and `i` is a number. Confirmed by reading the source
against the resulting behaviour: with `n` omitted, `array` starts as `[]` (not pre-sized), and
setting `array.length = i` at the end is a no-op, since the array already has length `i`. The guard
doesn't express what it appears to intend, but there is no input that makes it produce a wrong
result.

`set.js` was also read end to end: 356 lines, no shared mutable state, no typed arrays, no index
arithmetic, no re-entrancy hazards, and nothing to file. Not every file in this library has a bug in
it.
