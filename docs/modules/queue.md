...
| DIV-STACK-8 | **napi's generator `#.return` is deleted from this class's cursors, and from the four other sequence classes' — not from the port's other 47.** | Upstream cursors have none, so `break` leaves the walk resumable. The rest of the port's cursors keep napi's `return` and do not; see `docs/modules/stack.md` and `docs/DIVERGENCES.md`. |
