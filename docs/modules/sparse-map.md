...
`values()` still yields real data, both pointer widths at both boundaries, and cursor
non-restartability across all three projections. Full test-to-gap mapping: evidence file.
...
| — | **`new()` returns `Result`, and validates before allocating.** | Upstream throws for `length > 2³²`, and reaches that throw inside `getPointerArray` *before* `new Values(length)` runs. |
...
Two campaigns, two seeds, **2.65 M operations, zero divergences**:
...
Reproduce with `target/release/difffuzz --module sparse-map --seed 42 --cases 17268`.
...
`length`, `keys()` gaps on `dense`, `values()` may still have real data from a grown `Array` store,
and `entries()` never gaps at all; folding them into one op would leave every such program still
passing. Full grammar: evidence file.

`$forEach(method, rule, limit)` walks the instance with a callback that calls back into it. This
module's mutations are `delete(a1)`, `set(a1, a0)` and `clear()`, all uncapped — `set` writes back
the pair it was just handed, so it overwrites rather than grows and is safe uncapped even though
this module's bound, like `sparse-set`'s, is live. The callback argument order matters here: upstream
passes the value first and the key second, which is why `set`'s rule is `arg1,arg0`. What it does not
reach: the napi bridge, where a re-entrant callback would actually run, is outside the loop
`difffuzz` compares; `tests/boundary/reentrancy.js` covers that instead. One deliberate narrowing,
mirrored on both sides: a selected callback argument that is `undefined` skips the mutation, because
feeding it back in reaches upstream's `NaN`-indexed swap, which `usize` cannot express and the core
does not model. Disclosed in `fuzz/log.txt`.

**The fuzzer was falsified twice, once per half of this module's new grammar, and both leave the
original upstream suite green.** Sabotage A "fixes" `delete` to move the value along with the key —
not a typo, the change a reader makes on purpose having decided the omission was an oversight — and
is caught in 1,474 cases (3.0 s), shrunk to three ops; the same sabotage leaves upstream's mocha
suite at 9 passing, 0 failing, which is the most instructive seed in the repo. Sabotage B makes the
`Array` value store unable to grow, matching the typed store's dropped-write behaviour — the natural
Rust assumption, since a `Vec` allocated at `length` has a length — and is caught in 1,147 cases
(4.6 s), shrunk to a single operation past the constructor. Both reverted; both seeds committed in
`crates/difffuzz/proptest-regressions/sparse-map.txt`, replayed before any novel case on every
subsequent run. Full repro code: evidence file.

**Falsification of the port (gate 6), separate from the fuzzer falsifications above:** the assertion
named first was `should be possible to create an iterator over the map's values` —
`assert.deepStrictEqual(obliterator.take(map.values()), [13, 22, 8])` at `test/sparse-map.js:127` —
chosen because it reaches the projection machinery this unit adds. The sabotage — the `Values`
