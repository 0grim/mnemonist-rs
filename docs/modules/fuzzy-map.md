...
| # | Divergence | Why |
|---|---|---|
| — | **Core stores `Option<V>`, not `V`.** | `this.items.get(key)` is `undefined` for both "no such key" and "the key holds `undefined`", exactly as `default-map`. `None` spells the latter; `has`/`get` diverge on it the same way upstream's do. |
| — | **Hashing lives entirely in the bridge.** | The hash function(s) are JS callbacks; core takes the already-hashed key, the same split `default-map`'s factory makes. `crates/mnemonist-napi/src/fuzzy_map.rs`'s `HashFn` is `FunctionRef<Unknown<'static>, Unknown<'static>>` rather than a typed signature, because `add`'s hash argument is genuinely unconstrained (upstream's own test hashes a bare object). |
| — | **A falsy descriptor slot becomes `None`, not a stored `identity` closure.** | `if (!this.writeHashFunction) this.writeHashFunction = identity;` is a truthiness test (`0`, `''`, `false`, `null` all fall through), not a null check. `resolve_hash` mirrors the truthiness test; `None` means "classify the value directly," which is observably identical to calling a real `identity` and feeding its return into `JsKey::from_unknown`, without paying for a `FunctionRef` and a JS round trip for what is a no-op. |
| — | **`forEach`'s second callback argument is the value, not a hashed key.** | Reproduces the exact one-parameter delegation shown above; both core's `values_mut`/cursor step and the bridge's `for_each` project the *value* out twice. Not tested upstream (gap 1 above), but changing it would be wrong regardless. |
| — | **`inspect()` is not ported.** | A Node display convenience with no upstream assertion. |
| — | **The `[write, read]` array-descriptor form is excluded from the fuzz grammar.** | It needs two independent named factories per case; the single-function form is what the campaign spends its budget on, and the pair form is covered instead by `FuzzyMap.from`'s own upstream test and by `mnemonist_napi::fuzzy_map`'s construction tests. Disclosed rather than silently narrowed. |
| — | **Fuzzed items are always strings, never objects.** | A hash function that can throw (`item.title.toLowerCase()` on a bare string) would turn every non-title-bearing generated item into an apparatus failure rather than a comparison; `identity`/`lower` both accept a bare string, keeping every generated program well-defined on both sides. |
