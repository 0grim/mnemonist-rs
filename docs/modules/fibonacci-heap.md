...
10. `FibonacciHeap.MinFibonacciHeap` (`FibonacciHeap` itself under an alias) and the
    ...
    "Bugs this found", BUG-FIBONACCI-HEAP-2.
...
mid-`pop` without panicking (pinning BUG-FIBONACCI-HEAP-1's exact `-1` rather than merely "doesn't crash"), plus a
...
Three upstream defects, **BUG-FIBONACCI-HEAP-1 through BUG-FIBONACCI-HEAP-3**, verified either by tracing upstream's own
deterministic control flow or by the differential fuzzer itself.

**BUG-FIBONACCI-HEAP-1** — a comparator that `clear()`s the heap from inside a `pop`'s `consolidate` leaves
...
**BUG-FIBONACCI-HEAP-2** — `MaxFibonacciHeap.prototype = FibonacciHeap.prototype` — the identical
`instanceof`-blurring anti-pattern `heap.js`'s BUG-HEAP-4 already documents, one file over.

**BUG-FIBONACCI-HEAP-3** — a `clear()` fired from inside a **`push`'s** tie-break (not a `pop`'s `consolidate` —
BUG-FIBONACCI-HEAP-1's site) leaves `root` `null` while `min` is restored to a real node by that same `push`'s own
...
**BUG-FIBONACCI-HEAP-1 is the one worth reading twice**, because the two halves are almost independent bugs:
...
| # | Divergence | Why |
|---|---|---|
| DIV-FIBONACCI-HEAP-1 | **`MaxFibonacciHeap` is installed as evaluated JavaScript, not a second native class.** | Reproduces BUG-FIBONACCI-HEAP-2's prototype-sharing exactly, the same mechanism `crate::heap`'s `install_heap_statics` already uses for `MaxHeap`; a second `#[napi]` class would silently repair the `instanceof` blur instead. |
| DIV-FIBONACCI-HEAP-2 | **`size` is `i64`, not `usize`.** | `usize` cannot represent BUG-FIBONACCI-HEAP-1's `-1`; clamping or panicking on the arithmetic itself would each be a different, more "defensive" behaviour than upstream's own silent corruption. Matches `multi-set`'s DIV-MULTI-SET-1 precedent. |
| DIV-FIBONACCI-HEAP-3 | **BUG-FIBONACCI-HEAP-1/BUG-FIBONACCI-HEAP-3's crashes are Rust panics whose message IS upstream's exact `TypeError` text.** | `mnemonist-core` has no exceptions. The message text (not a description of the Rust-side invariant) is what lets the fuzz harness recover the exact upstream wording without a hand-maintained translation table. Noted inconsistency with `_utils`'s DIV-UTILS-1, which chose `Result<_, KWayError>` for BUG-UTILS-1 instead — that call site already returns a `Result` its callers handle routinely; this one is reachable only through one adversarial re-entrant sequence, and threading a new error variant through every `pop`/`consolidate` caller for it was judged disproportionate. |
| DIV-FIBONACCI-HEAP-4 | **Nodes live in an arena of `NodeId`s, never `Rc<RefCell<Node>>`, and a popped slot is never recycled.** | A literal `Rc` translation of upstream's circular doubly-linked list is a strong-reference cycle that never reaches zero; a *recycling* arena panics or silently aliases two nodes under re-entrant nested `pop`s (see "Bugs this found"). Both are unobservable through the public API either way — nothing exposes node identity or arena occupancy. |
...
defect), `fibClearer` (a re-entrant clear — BUG-FIBONACCI-HEAP-1/BUG-FIBONACCI-HEAP-3's trigger). Observable state is `size` and
`peek`, compared as the signed `i64` both sides can produce, since a campaign that clamped or
ignored negative values would have missed BUG-FIBONACCI-HEAP-1 entirely. Full grammar: evidence file.

**Both crash sites (BUG-FIBONACCI-HEAP-1, BUG-FIBONACCI-HEAP-3) fire constantly across a normal campaign** — visible directly in
...
`merge_k_matches_upstreams_real_heap_on_the_case_that_found_div_utils_2`) caught it too, since DIV-UTILS-2's fix
