...
Native tests transcribing upstream's own cases as a baseline, closing every gap above: explicit
bounds windowing the search (including empty windows and `hi == 0`), an over-long `hi` reporting a
hit at a hole (plus the two bounds' opposite reactions), `lowerBoundIndices` defaulting `hi` from
the wrong array, empty arrays across all seven functions, duplicates pinning the midpoint
arithmetic, `NaN` reported as a match by `search`, unsorted input's deterministic behaviour, an
antisymmetric comparator hiding the argument-order difference between the two comparator families,
and the underlying property checked exhaustively: every non-decreasing array of length 0..=8 over
`{0,1,2}` crossed with every needle in `-1..=3`, all six bound/search functions against a linear
scan — 3,280 arrays, where upstream checks two. Full test-to-gap mapping: evidence file.
...
the native suite. Two sabotages, each with its target assertion named **before** the run, are
confirmed red at exactly the named assertion — deleting the `undefined`-read model breaks
`an_over_long_hi_reports_a_hit_at_a_hole` with an out-of-bounds panic while every other test still
passes, and deleting `search`'s "neither greater nor less means equal" rule breaks
`nan_is_reported_as_a_match_by_search` plus five others including
`search_matches_the_upstream_suites_own_case`. Both reverted; 15/15 green afterwards. The first is
the interesting one: it fails *only* the assertion it was aimed at, which is what distinguishes a
real falsification from a sabotage so broad that any test would have caught it. Full record:
evidence file.
